...
class GameController:
    ...
    def HandleEventUI(self):
        ...
        if self.state == GameState.INIT:
            ...
            if Button(1200, 640, self.aboutUsButton, 0.5).draw(self.screen):
                webbrowser.open(
                    "https://drive.google.com/file/d/1njq8S15yb5eUZwvlXC8dDCMySusKQqPH/view?usp=sharing"
                )
                # will change
        ...
        if self.state == GameState.END:
            self.endTemplate = pygame.transform.scale(
                self.settingTemplate,
                (
                    int(self.settingTemplate.get_width() * 1.3),
                    int(self.settingTemplate.get_height() * 1.3),
                ),
            )
    ...
    def Play(self):
        ...
        self.exitButtonMenu = pygame.image.load(
            "../asset/icon/asset/exit_button_menu.png"
        )
        self.settingIcon = pygame.image.load("../asset/icon/asset/setting_icon.png")
        self.volumeActiveIcon = pygame.image.load(
            "../asset/icon/asset/volume_active.png"
        )
        self.volumeDisableIcon = pygame.image.load(
            "../asset/icon/asset/volume_disable.png"
        )
        self.musicActiveIcon = pygame.image.load("../asset/icon/asset/sound_active.png")
        self.musicDisableIcon = pygame.image.load(
            "../asset/icon/asset/sound_disable.png"
        )
        self.settingTemplate = pygame.image.load(
            "../asset/icon/asset/setting_template.png"
        )
        self.resumeButton = pygame.image.load("../asset/icon/asset/resume_button.png")
        self.exitButtonSetting = pygame.image.load(
            "../asset/icon/asset/exit_button_setting.png"
        )
        ...
        self.exitPopupTemplate = pygame.image.load(
            "../asset/icon/asset/exit_popup_template.png"
        )
        ...
        for bullet in self.bulletManager.bulletList:
            for player in self.mainPlayers.players:
                if player.hit(bullet):
                    ...
                    if (
                        self.mainPlayers.players.__len__() - 1 != 0
                        and player == self.mainPlayers.currentPlayer
                    ):
            ...
            for player in self.extraPlayers.players:
                if player.hit(bullet):
                    if (
                        self.extraPlayers.players.__len__() - 1 != 0
                        and player == self.extraPlayers.currentPlayer
                    ):
